...
/*SELECT f.title , count(f.rental_duration) AS "Most Rented"
FROM film f 
JOIN inventory i 
...
GROUP BY f.film_id
ORDER BY count(f.rental_duration) desc;*/
...
/*EXPLAIN SELECT A.customer_id, concat(A.first_name,' ',A.last_name) AS Full_name, sum(B.amount) as Spendings
FROM customer as A  
...
Explain plans usually gives step by step process of how our query will execute in the postgress, time needed, cost of the query. 
These are basically used while doing performance tuning queries to reduce load on the servers.
...
/*Explain plan for 5:*/
/*Explain SELECT count(film.title) As movies_done, 
concat(actor.first_name,' ',actor.last_name)as Actor_fullname ,film.release_year
FROM film
INNER JOIN  film_actor
ON film.film_id=film_actor.film_id
